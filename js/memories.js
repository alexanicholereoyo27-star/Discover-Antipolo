// 📸 Discover Antipolo Memories - PRODUCTION VERSION (FIXED AUTH)
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function initializeFirebase() {
        try {
            // ✅ Your Firebase project configuration (SAME AS MAIN APP)
            ...
            // Check if Firebase is already initialized (from main app)
            const existingApps = getApps();
            if (existingApps.length > 0) {
                // Use the existing Firebase app (from your main application)
                app = existingApps[0];
                console.log("✅ Using existing Firebase app");
            } else {
                // Initialize new app if no existing one found
...
`;  
        ...
        // ✅ Upload memory with auth check - FIXED VERSION
function setupUploadForm() {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        // Manual validation instead of relying on HTML5 required attributes
        if (!spot) {
        ...
        if (!selectedFile) {
            ...
            // Show the file picker when no file is selected
